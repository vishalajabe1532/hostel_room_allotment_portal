...
<?php
   if (isset($_POST['search'])) {
   	   ...
   	   $query_search = "SELECT * FROM Application WHERE MIS like '$search_box%' and Hostel_id = '$hostel_id' and Application_status = '1'";
    ...
    <?php
   	   if(mysqli_num_rows($result_search)==0){
            ...
            $query7 = "SELECT * FROM Students WHERE MIS = '$MIS'";
            $result7 = mysqli_query($conn,$query7);
            $row7 = mysqli_fetch_assoc($result7);
            $student_name = $row7['Fname']." ".$row7['Lname'];
            
      		echo "<tr><td>{$student_name}</td><td>{$row_search['MIS']}</td><td>{$hostel_name}</td><td>{$row_search['Message']}</td></tr>\n";
         ...
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $student_name = $row7['Fname']." ".$row7['Lname'];
      		...
      		echo "<tr><td>{$student_name}</td><td>{$row1['MIS']}</td><td>{$hostel_name}</td><td>{$row1['Message']}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
...
if(isset($_POST['submit'])){
   ...
   while($row1 = mysqli_fetch_assoc($result1)){
     ...
     $MIS = $row1['MIS'];
     $query3 = "UPDATE Application SET Application_status = '0',Room_No = '$room_no' WHERE MIS = '$MIS'";
     $result3 = mysqli_query($conn,$query3);
     /*echo "<script type='text/javascript'>alert('<?php echo $result3; ?>')</script>";*/
     if($result3){
     	$room_id = $row2['Room_id'];
     	$query4 = "UPDATE Student SET Hostel_id = '$hostel_id',Room_id = '$room_id' WHERE MIS = '$MIS'";
