	...
	<header>
		<div class="container agile-banner_nav">
			<nav class="navbar navbar-expand-lg navbar-light bg-light">
				...
				<h1><a class="navbar-brand" href="admin_home.php">COEP <span class="display"> </span></a></h1>
...
<?php
   if (isset($_POST['search'])) {
   	   ...
   	   $query_search = "SELECT * FROM Students WHERE MIS like '$search_box%'";
         ...
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $student_name = $row1['Fname']." ".$row1['Lname'];
      		...
      		echo "<tr><td>{$student_name}</td><td>{$row1['MIS']}</td><td>{$row1['Mob']}</td><td>{$HNM}</td><td>{$room_no}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
