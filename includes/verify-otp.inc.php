...
if (isset($_POST['verify_otp'])) {
  if(isset($_SESSION['otp'])  && isset($_POST['otp'])){
    ...
    if($submitted_otp == $stored_otp && (time() - $otp_timestamp) <= 300){
        ...
        echo "<script type='text/javascript'>alert('Invalid OTP or OTP has expired. Please try again.'); window.location.href='../signup.php?error=otpexpired';</script>";
        
      }
    }
    else {
      echo "<script type='text/javascript'>alert('OTP not found.'); window.location.href='../signup.php?error=otpnotfound';</script>";
